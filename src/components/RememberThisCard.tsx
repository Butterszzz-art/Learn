...
export function RememberThisCard({ due }: { due: DueReviewTopic }) {
  ...
  return (
    <div className="card mb-6 border-brain-accent2/40 bg-gradient-to-br from-brain-surface to-brain-surface2">
      <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-brain-accent2">
        🧠 Remember this?
      </p>
      <p className="mb-3 text-xs text-brain-muted">
        {due.interestName} · covered {due.coveredDaysAgo === 0 ? "today" : `${due.coveredDaysAgo}d ago`}
      </p>
      <p className="mb-3 font-serif text-lg leading-snug">{due.topic}</p>
      ...
      {!revealed ? (
        ...
        <div className="space-y-3">
          {due.contentPreview ? (
            <p className="text-sm leading-relaxed text-brain-text/90">{due.contentPreview}</p>
          ) : (
          ...
          {due.deepDiveId && (
            <Link href={`/deep-dive/${due.deepDiveId}`} className="inline-block text-xs text-brain-accent hover:underline">
